...
export const UtilityCommand = () => {
    const buttons = [
        { icon: Landmark, bg: 'bg-[#3B82F6]', border: 'border-[#1D4ED8]', label: 'Tour' },
        { icon: Briefcase, bg: 'bg-[#A855F7]', border: 'border-[#7E22CE]', label: 'Bag' },
        { icon: Zap, bg: 'bg-[#EAB308]', border: 'border-[#A16207]', label: 'Pulse' },
        { icon: Timer, bg: 'bg-[#64748B]', border: 'border-[#334155]', label: 'Ledger' },
    ];
    ...
    return (
        <div className="absolute left-6 top-1/2 -translate-y-1/2 flex flex-col gap-5 z-40 pointer-events-auto hidden lg:flex">
            {buttons.map((btn, idx) => (
                <button
                    key={idx}
                    className={`group relative w-14 h-14 rounded-full ${btn.bg} ${btn.border} border-b-[5px] border-x-[2px] border-t-[1px] sticker-shadow flex items-center justify-center transition-all duration-200 hover:scale-110 active:translate-y-[3px] active:shadow-[0_2px_0px_#0f172a] active:border-b-[2px]`}
                >
